...
flashcache_writeback_create(struct cache_c *dmc, int force)
{
	...
	int sectors_written = 0, sectors_expected = 0; /* debug */
	int slots_written = 0; /* How many cache slots did we fill in this MD io block ? */
	printk("flashcache_writeback_create by call\n");
	...
	printk("flashcache_writeback_create by call flashcache-dbg: cachedev check - %s %s", header->cache_devname,
				dmc->dm_vdevname);
...
{
	struct cache_c *dmc = (struct cache_c *) ti->private;
    //printk("flashcache_status by call!\n");
	switch (type) {
			   ...
			   iterate_devices_callout_fn fn, void *data)
{
	struct cache_c *dmc = (struct cache_c *) ti->private;
    int ret = 0;
    //printk("flashcache_iterate_devices by call!\n");
...
flashcache_sync_for_remove(struct cache_c *dmc)
{
	do {
		...
		//zkm add for hot-swap
		wait_event_timeout(dmc->destroyq, !atomic_read(&dmc->nr_jobs), 5*HZ);
		//zkm add for hot-swap
