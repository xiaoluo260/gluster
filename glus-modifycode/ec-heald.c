...
int
ec_shd_file_count(xlator_t *subvol, gf_dirent_t *entry, loc_t *parent,
                   void *data)
{
        int* pdata = (int*) data;
        *pdata = *pdata + 1;
...
ec_shd_full_sweep (struct subvol_healer *healer, inode_t *inode)
{
        ...
        int              fd = -1;
        int             cnt = 0;
        const char    *file = "/var/log/glusterfs/ec.heal_log";
        char        msg[30] = "";

        ec = healer->this->private;
        loc.inode = inode;

        fd = open (file, O_CREAT | O_RDWR | O_APPEND | O_TRUNC, S_IRUSR | S_IWUSR);

        if (fd < 0) {
                gf_msg ("wxb ng", GF_LOG_INFO, 0,
                        EC_MSG_FULL_SWEEP_START,
                        "ERROR: failed to create ec log file");
        }

        loc.fd = -1;
        syncop_ftw (ec->xl_list[healer->subvol], &loc,
                           GF_CLIENT_PID_SELF_HEALD, &cnt,
                           ec_shd_file_count);

        loc.fd = fd;
        snprintf(msg, 30, "%d\n", cnt);
        sys_write (loc.fd, msg, strlen(msg));
        ret =  syncop_ftw (ec->xl_list[healer->subvol], &loc,
                           GF_CLIENT_PID_SELF_HEALD, healer,
                           ec_shd_full_heal);

        if (ret) {
                snprintf(msg, 30, "fail\n");
                sys_write (loc.fd, msg, strlen(msg));
        } else {
                snprintf(msg, 30, "end\n");
                sys_write (loc.fd, msg, strlen(msg));
...
//add for fuse heal
int
ec_shd_healer_spawn1 (xlator_t *this, struct subvol_healer *healer,
                      void *(threadfn)(void *))
{
        int ret = 0;

        pthread_mutex_lock (&healer->mutex1);
        {
                if (healer->running1) {
                        pthread_cond_signal (&healer->cond1);
                } else {
                        ret = gf_thread_create (&healer->thread1, NULL,
...
unlock:
        pthread_mutex_unlock (&healer->mutex1);
...
//add for fuse heal
//	ec_shd_healer_spawn1 (this, NTH_FULL_HEALER (this, subvol),
//			      ec_shd_full_healer);
//add for fuse heal
...
{
        // add for fuse heal
        if (strcmp(ec->mount_point + 1, ec->xl->name) == 0)
	{
                ec_shd_full_healer_spawn (ec->xl, child);
	}
...
int
shdfuse_ec_heal_op (xlator_t *this)
{
        char key[64] = {0};
        int op_ret = 0;
        ec_t *ec = NULL;
        ...
        op_ret = -1;
        for (i = 0; i < ec->nodes; i++) {
                snprintf (key, sizeof (key), "%d-%d-status", ec->idx, i);
                if (((ec->xl_up >> i) & 1) == 0) {
                        //ret = dict_set_str (output, key, "Brick is not connected");
                        //gf_msg (this->name, GF_LOG_INFO, 0 ,GLFS_EC_COMP_BASE ,"zkm error xl_up is %d", ec->xl_up);
                        ret = 0;
                } else if (!ec->up) {
                        //gf_msg (this->name, GF_LOG_INFO, 0 ,GLFS_EC_COMP_BASE ,"zkm error ec->up is %d", ec->up);
                        //ret = dict_set_str (output, key,
                         //                   "Disperse subvolume is not up");
                        ret = 0;
               /* } else if (!ec_shd_is_subvol_local (this, i)) {
